...
2. **World-context** — скан NPC, областей, квестов и собственных имён для согласованности перевода; по существам, плейсам и дверям на картах определяется, кто говорит реплики диалогов.
...
Поля одного NPC или предмета и записи одной категории журнала переводятся связанными группами с общим контекстом. Небольшие группы объединяются в запросы с явными границами; крупные разделяются по бюджету текста, контекста и глоссария. Части диалогов сохраняют переходы и контекст соседних узлов. NCS-строки группируются по скрипту после проверки безопасности; порядок констант не считается порядком исполнения.
...
- В web-редакторе у каждой реплики диалога видно, кто её говорит: NPC или объект (имя и тег), игрок или владелец диалога.
