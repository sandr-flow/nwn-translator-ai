...
def _build_script_message_profile_rules(target_lang: str, gender: str) -> str:
    ...
    return (
        ...
        "4. Tokens like <VAR1> and <VAR2> are runtime values the game concatenates "
        "(names, numbers). Keep them and place them where the target-language grammar "
        "requires.\n"
        "5. The translated text MUST be grammatically correct and must fit as a short "
        "in-game message.\n"
...
def build_dialog_system_prompt_parts(
    ...
    stable = (
        ...
        "8. Builder placeholders that appear as in-game reply text (for example "
        "END DIALOG, [END], CONTINUE, or a lone ...) must become a short natural "
        "reply in the target language. Never return an empty translation. "
        "If the meaning is unclear, return the original text unchanged.\n"
        f"9. {_proper_names_rules(target_lang)}"
        f"10. {_speech_style_rules(target_lang)}"
