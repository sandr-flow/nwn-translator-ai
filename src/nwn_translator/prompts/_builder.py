...
def _proper_names_rules(target_lang: str, *, glossary_rule: str = "") -> str:
...
def _speech_style_rules(target_lang: str) -> str:
    ...
    example_block = "\n".join(
        f'    - "{eng}" -> "{good}" '
        f'(GOOD, broken) \u2014 NOT "{bad}" (BAD, normalized)'
...
def build_translation_system_prompt(
    ...
    return (
        ...
        f"Exception: see rule 11 for intentionally broken speech.\n"
        f"7. {_proper_names_rules(target_lang, glossary_rule=glossary_rule)}"
...
def build_glossary_system_prompt(target_lang: str) -> str:
    ...
    pers_ex = ", ".join(
        f'"{eng}" -> "{tr}"' for eng, tr in personal
    )
    desc_ex = ", ".join(
        f'"{eng}" -> "{good}" (NOT "{bad}")'
        for eng, good, bad in descriptive
