...
void SetItemLocals()
{
SetLocalString(OBJECT_SELF, "lsn1", "tableleg");
SetLocalInt(OBJECT_SELF, "lsc_tableleg", 1);
SetLocalString(OBJECT_SELF, "lsi1_tableleg", "pileofwood");
SetLocalInt(OBJECT_SELF, "lss_tableleg", -10);
SetLocalInt(OBJECT_SELF, "lsv_tableleg", -10);
...
for (nLoop=1; nLoop<=lsn; nLoop++)
   ...
   if (bOkay==TRUE)
      ...
      CreateItemOnObject(sCur, oOwner);
      int nVis1=GetLocalInt(OBJECT_SELF, "lsv_"+sCur);

      object oInvObject = GetNearestObjectByTag( "Playbanging" );
      AssignCommand( oInvObject, PlaySound("as_cv_sawing1") );
