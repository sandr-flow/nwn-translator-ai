...
void LookUpWalkWayPoints()
{
    ...
    int nNth=1;
    object oWay = GetNearestObjectByTag(sTag + GetWaypointSuffix(nNth));
...
object GetNextWalkWayPoint(object oCreature=OBJECT_SELF)
{
    ...
    return GetLocalObject(oCreature, sPrefix + IntToString(nCurWay));
}
...
void WalkWayPoints(int nRun = FALSE, float fPause = 1.0)
{
  ...
  //  SpeakString("in my waypoint walking");
    ...
    if (GetIsObjectValid(oWay) == TRUE)
    {
        ...
        // Pause there briefly even if we get triggered again
        ActionWait(fPause);
        ActionDoCommand(WalkWayPoints());      // February 14 2003 added else route only happens once
