...
int chooseTactics(object oIntruder)
{
   ...
   // **************************************
    ...
    if (InvisibleTrue(OBJECT_SELF) == TRUE && nMagic > 0)
    {
        ...
        //nOffense = 7;     // Baldecaran: I turned this off because invisible
                            // creatures became unbearably passive, including Llarien.
        nMagic = 100;
    ...
    if (TalentRangedEnemies(oIntruder)) {
        //SendMessageToPC(GetFirstPC(),"DEBUG: Ranged attack!");  // DEBUGOFF
        return 99;}    // Added by Baldecaran
...
void DetermineSpecialBehavior(object oIntruder = OBJECT_INVALID)
{
    ...
    if (!GetIsObjectValid(oTarget) || GetDistanceToObject(oTarget)>15.0)
        oTarget = GetNearestCreature(CREATURE_TYPE_IS_ALIVE,TRUE); // Make herbivores respond to non-enemies
    ...
    else if(GetBehaviorState(NW_FLAG_BEHAVIOR_HERBIVORE))
    {
        if(!GetIsObjectValid(GetAttemptedAttackTarget()) &&
           ...
           !GetIsObjectValid(GetAttackTarget()))
        {
            if(GetIsObjectValid(oTarget) && GetDistanceToObject(oTarget) <= 12.0)
            {
                //SendMessageToPC(GetFirstPC(),"Herbivore "+GetName(OBJECT_SELF)+" sees "+GetName(oTarget)); // DEBUGOFF
                if(!GetIsFriend(oTarget) && !GetLocalInt(oTarget,"bHerbivore"))
                {
                    if(GetLevelByClass(CLASS_TYPE_DRUID, oTarget) == 0 && GetLevelByClass(CLASS_TYPE_RANGER, oTarget) == 0)
                    {
                        TalentFlee(oTarget);
