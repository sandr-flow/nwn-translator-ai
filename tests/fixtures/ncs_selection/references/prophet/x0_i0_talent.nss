...
int TalentUseTurning()
{
    ...
    if(GetHasFeat(FEAT_TURN_UNDEAD))
    {
        ...
        // Do not turn if there are friendly undead nearby
        object oFriendUndead = GetNearestCreature(CREATURE_TYPE_RACIAL_TYPE,RACIAL_TYPE_UNDEAD,
            OBJECT_SELF,1,CREATURE_TYPE_REPUTATION,REPUTATION_TYPE_FRIEND);
        if (GetIsObjectValid(oFriendUndead) && GetDistanceToObject(oFriendUndead)<15.0)
            return FALSE;
        oFriendUndead = GetNearestCreature(CREATURE_TYPE_RACIAL_TYPE,RACIAL_TYPE_UNDEAD,
            OBJECT_SELF,1,CREATURE_TYPE_REPUTATION,REPUTATION_TYPE_NEUTRAL);
        if (GetIsObjectValid(oFriendUndead) && GetDistanceToObject(oFriendUndead)<15.0)
            return FALSE;
