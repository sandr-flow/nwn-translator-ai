    ...
    int nMaxLevelInSecondClass=0, int nPackageClass1=PACKAGE_INVALID, int nPackageClass2=PACKAGE_INVALID)
{
    ...
    while ( (nLevel1 + nLevel2) < nLevel )
    {
        ...
        if (nLevel2 > nMaxLevelInSecondClass && nMaxLevelInSecondClass != 0)
        {
            nClassToLevelUp = nClass1;
            nPackageToUse = nPackageClass1;
        }
    ...
    for (i=1; i<= GetMaxHenchmen(); i++)
    {
        ...
        if ( GetIsObjectValid(oAssociate) )
        {
            ...
            if (GetLocalInt(oAssociate, "X2_JUST_A_FOLLOWER") == FALSE)
            {
                ...
                if(sTag == "giles")
                {
                    if(GetLocalInt(GetModule(), "GilesTransformed"))
                        LevelHenchmanUpTo(oAssociate, nLevel - 1, CLASS_TYPE_FIGHTER, 0, PACKAGE_INVALID, PACKAGE_FIGHTER);
                    else
                        LevelHenchmanUpTo(oAssociate, nLevel - 1);
                }
                else if(sTag == "reed")
                {
                    LevelHenchmanUpTo(oAssociate, nLevel - 1, CLASS_TYPE_SHADOWDANCER, 10, PACKAGE_INVALID, PACKAGE_SHADOWDANCER);
                }
                else if(sTag == "chantilly")
                {
                    if(GetLocalInt(GetModule(), "ChantillyTransformed"))
                        LevelHenchmanUpTo(oAssociate, nLevel - 1, CLASS_TYPE_ARCANE_ARCHER, 10, PACKAGE_INVALID, PACKAGE_ARCANE_ARCHER);
                    else
                        LevelHenchmanUpTo(oAssociate, nLevel - 1);
                }
                else if(sTag == "loveinterest_f")
                {
                    if(GetLevelByClass(CLASS_TYPE_FIGHTER, oAssociate) == 11)
                        SetLocalInt(oAssociate, "X0_L_LEVELRULES", 1);
                    LevelHenchmanUpTo(oAssociate, nLevel - 1, CLASS_TYPE_WEAPON_MASTER, 0, PACKAGE_INVALID, PACKAGE_WEAPONMASTER);
                }
                else
                {
                    LevelHenchmanUpTo(oAssociate, nLevel - 1);
